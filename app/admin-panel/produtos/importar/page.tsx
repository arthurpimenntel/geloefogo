...
function ImportarProdutosContent() {
  ...
  function loadCategoryProducts(catId: string, page = 1) {
    setCatProductsLoading(true)
    fetch(`/api/cj?action=category-products&categoryId=${catId}&page=${page}&size=20`)
      .then(r => r.json())
      .then(d => {
        setCatProducts(d?.data?.list ?? [])
  ...
  function selectCategory(cat: CJCategory) {
    setSelectedCat(cat); setCatProducts([]); loadCategoryProducts(cat.categoryId, 1)
  }
  ...
  function ProductGrid({
    products, selected, onToggle, onToggleAll, onImport, importing, results, loading,
  }: {
                ...
                {!selectedCat ? (
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        {catProductsPage > 1 && (
                          <button onClick={() => loadCategoryProducts(selectedCat.categoryId, catProductsPage - 1)}
