  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="font-playfair text-2xl text-[#1C1008]">Novo Produto</h1>
          <p className="text-[#8C6D3F] text-xs mt-0.5">Cadastro manual de produto</p>
        </div>
      </div>

      {success && (
        <div className="mb-6 bg-green-50 border border-green-200 rounded-2xl px-5 py-3 text-green-700 text-sm">
          ✅ Produto cadastrado com sucesso! Redirecionando...
        </div>
      )}

      {error && (
        <div className="mb-6 bg-red-50 border border-red-200 rounded-2xl px-5 py-3 text-red-600 text-sm">
          ❌ {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Section title="Categorização">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="País de Origem">
              <input value={form.origin_country} onChange={e => set('origin_country', e.target.value)} placeholder="Cuba" className={INPUT} />
            </Field>
            <Field label="Tags" help="Separar por vírgula">
              <input value={form.tags} onChange={e => set('tags', e.target.value)} placeholder="Importado, Premium, Mais Vendido" className={INPUT} />
            </Field>
          </div>
        </Section>
        ...
        <Section title="Preços">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Field label="Preço de Venda (R$) *">
              <input type="number" step="0.01" min="0" value={form.sale_price} onChange={e => set('sale_price', e.target.value)} placeholder="890.00" required className={INPUT} />
            </Field>
            <Field label="Preço Comparação (R$)" help="Riscado">
              <input type="number" step="0.01" min="0" value={form.compare_price} onChange={e => set('compare_price', e.target.value)} placeholder="1050.00" className={INPUT} />
            </Field>
            <Field label="Custo (R$)">
              <input type="number" step="0.01" min="0" value={form.cost_price} onChange={e => set('cost_price', e.target.value)} placeholder="400.00" className={INPUT} />
            </Field>
          </div>
        </Section>
        ...
        <Section title="Estoque e Logística">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Peso (gramas)">
              <input type="number" min="0" value={form.weight_g} onChange={e => set('weight_g', e.target.value)} placeholder="250" className={INPUT} />
            </Field>
          </div>
        </Section>
          ...
          {form.images && (
            <div className="flex gap-2 mt-3 flex-wrap">
              {form.images.split(',').map(url => url.trim()).filter(Boolean).map((url, i) => (
                <div key={i} className="w-16 h-16 border border-[#E8DCC8] rounded-xl overflow-hidden">
        ...
        <Section title="Status">
          <label className="flex items-center gap-3 cursor-pointer">
            <div
              ...
              className={`w-10 h-5 rounded-full relative transition-colors ${form.active ? 'bg-[#C08D3A]' : 'bg-[#D9C9A8]'}`}
            >
            ...
            <span className="text-[#6B4F2A] text-sm">
        ...
        <div className="flex gap-4 pt-4">
          ...
          <button type="button" onClick={() => router.back()}
            className="px-6 py-3 border border-[#D9C9A8] rounded-xl text-[#6B4F2A] hover:border-[#C08D3A] hover:bg-[#F5EFE6] text-xs uppercase tracking-widest transition-colors">
            Cancelar
          </button>
        </div>
      </form>
...
const INPUT = `bg-white border border-[#E8DCC8] rounded-xl text-[#1C1008] px-4 py-2.5 text-sm
  focus:outline-none focus:border-[#C08D3A] transition-colors
  placeholder:text-[#B0916A] w-full`

function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-white border border-[#E8DCC8] rounded-2xl shadow-sm p-6">
      <h2 className="text-[#8C6D3F] text-[10px] uppercase tracking-[0.25em] mb-5">{title}</h2>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        <label className="text-[#8C6D3F] text-[11px] uppercase tracking-widest font-medium">{label}</label>
